...
pattern = r"(?<=(/|\=))[A-Z][a-zA-Z][a-zA-Z]+(?=\1)"  # without positive lookaround r"(=|/)(?P<dest>[a-zA-Z]{2,})\1"
text = input()

valid_destination = [d.group() for d in re.finditer(pattern, text)]
travel_points = sum([len(d) for d in valid_destination])
